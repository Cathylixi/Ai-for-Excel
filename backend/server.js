...
try {
  ...
  console.log('✅ 已挂载 /api 业务路由');
} catch (e) {
  console.warn('⚠️ 挂载 /api 业务路由失败:', e.message);
}

// 挂载助手路由（AI解析与确认流程）
try {
  const assistantRoutes = require('./routes/assistantRoutes');
  app.use('/api/v2', assistantRoutes);
  console.log('✅ 已挂载 /api/v2 助手路由');
} catch (e) {
  console.warn('⚠️ 挂载 /api/v2 助手路由失败:', e.message);
}
...
function tryStartHttps() {
  try {
    ...
    httpsServer.listen(PORT, () => {
      console.log('🔐 使用 dev 证书启动 HTTPS 成功');
    ...
    console.warn('⚠️ 启动 HTTPS 失败，将回退到 HTTP:', err.message);
    return false;
  }
}
